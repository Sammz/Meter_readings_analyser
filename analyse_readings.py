...
tariffs = {date(2024, 5, 29): [0.2987, 0.1253, 0.4839], date(2024, 10, 8): [0.2882, 0.1209, 0.4921]}
price_peak_index      = 0
price_off_index       = 1
standing_charge_index = 2

price_peak_current      = 0
price_off_current       = 0
standing_charge_current = 0

def change_tariff(tariff_change_date):
    price_peak_current = tariffs[tariff_change_date][price_peak_index]
    price_off_current = tariffs[tariff_change_date][price_off_index]
    standing_charge_current = tariffs[tariff_change_date][standing_charge_index]


readings_file = "readings.txt"
output_file = "analysed_readings.csv"
...
def handle_missing_dates():
    ...
    for missing_day in missing_dates:
        ...
        if missing_day.day == 1:
            ...
            monthly_costs = round(
                (average_peak * price_peak_current + average_off * price_off_current + standing_charge_current) * month_length, 2)
            ...
            if "Off Peak" in line:
                ...
                if days > 0:
                    ...
                    if date_stats[date_index].day == 1:  # Record monthly average on a day on the first of the month
                        ...
                        date_stats[monthly_cost_index] = round((date_stats[average_peak_index] * price_peak_current + date_stats[average_off_index] * price_off_current + standing_charge_current) * month_length, 2)
                ...
                if start_date in tariffs.keys():
                    change_tariff(start_date)
...
# csv.append(headings)  # Add the headings again, such that they're on top and bottom of csv
csv.reverse()  # Put the readings in chronological order

# Calculate costs
for row in csv[:1]:
    if row[peak_usage_index] != '':
        if row[date_index] in tariffs.keys():
            change_tariff(row[date_index])
        row[costs_index] = round(float(row[off_usage_index]) * price_off_current + float(row[peak_usage_index]) * price_peak_current + standing_charge_current, 2)
